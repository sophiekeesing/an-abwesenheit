<template>
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Lehrer Dashboard</h2>
      <p class="text-gray-600 dark:text-gray-400">Verwalten Sie Fehlzeiten für Ihre Klassen</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      ...
      >
        ...
        <option v-for="classId in teacherClasses" :key="classId" :value="classId">
          {{ classId }}
        </option>
      </select>
    </div>
        ...
        >
          <div class="flex justify-between items-start">
            <div>
              ...
              <p class="text-sm text-gray-600 dark:text-gray-400">Klasse: {{ absence.classId }}</p>
...
const pendingAbsences = computed(() => {
  ...
  return selectedClass.value
    ...
    : pending.filter((absence) => teacherClasses.value.includes(absence.classId))
})
...
const filteredAbsences = computed(() => {
  ...
  const filtered = selectedClass.value
    ...
    : allAbsences.filter((absence) => teacherClasses.value.includes(absence.classId))
...
const approveAbsence = (absenceId: string) => {
  attendanceStore.updateAbsenceStatus(absenceId, 'approved', authStore.user?.id)
}

const rejectAbsence = (absenceId: string) => {
  attendanceStore.updateAbsenceStatus(absenceId, 'rejected', authStore.user?.id)
}
